...
void enableOutput(PWM_Handler_t *ptrPwmHandler) {
	...
	case PWM_CHANNEL_2: {
			// Activamos la salida del canal 2 (Capture/Compare 2 output enable)
...
void updateFrequency(PWM_Handler_t *ptrPwmHandler, uint16_t periodo){
    ...
    ptrPwmHandler->config.periodo = periodo - 1;
...
void setDuttyCycle(PWM_Handler_t *ptrPwmHandler){
	...
	case PWM_CHANNEL_1:{
		...
		ptrPwmHandler->ptrTIMx->CCR1 = (((ptrPwmHandler->config.periodo) * (ptrPwmHandler->config.duttyCicle))/100);

		break;
	}
	case PWM_CHANNEL_2: {

		ptrPwmHandler->ptrTIMx->CCR2 = (((ptrPwmHandler->config.periodo) * (ptrPwmHandler->config.duttyCicle))/100);

		break;
	}
	case PWM_CHANNEL_3: {

		ptrPwmHandler->ptrTIMx->CCR3 = (((ptrPwmHandler->config.periodo) * (ptrPwmHandler->config.duttyCicle))/100);

		break;
	}
	case PWM_CHANNEL_4: {

		ptrPwmHandler->ptrTIMx->CCR4 = (((ptrPwmHandler->config.periodo) * (ptrPwmHandler->config.duttyCicle))/100);
