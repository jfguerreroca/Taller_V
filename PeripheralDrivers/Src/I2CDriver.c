...
/**
 * Recordar que se debe configurar los pines para I2C (SDA ySCL),
...
void i2c_Config(I2C_Handler_t *ptrHandlerI2C){
	...
	/* 2. Reiniciamos el periférico, de forma que inicia en un estado conodido */
	ptrHandlerI2C->ptrI2Cx->CR1 |= I2C_CR1_SWRST;
	__NOP();
	ptrHandlerI2C->ptrI2Cx->CR1 &= -I2C_CR1_SWRST;

	/* 3. Indicamos cuall es la velocidad del reloj principal, que es la señal utilizada
	 * por el periférico para generar la señal de reloj para el bus I2C*/
	ptrHandlerI2C->ptrI2Cx->CR2 &= ~(0b111111 << I2C_CR2_FREQ_Pos);
	ptrHandlerI2C->ptrI2Cx->CR2 |= ~(MAIN_CLOCK_16_MHz_FOR_I2 << I2C_CR2_FREQ_Pos);
...
/* 7b. Activamos la indicación para ACK (indicación para el Slave de terminar) */
...
void i2c_sendSlaveAddressRW(I2C_Handler_t *ptrHandlerI2C, uint8_t slaveAddress, uint8_t readOrWrite ){
	 ...
	 * (esta nos indica que la dirección fue enviada satisfactoriamente */
...
uint8_t i2c_readSingleRegister(I2C_Handler_t *ptrHandlerI2C, uint8_t regToRead){
	...
	/* 3. Generamos la condición StartEnviamos la dirección de memoria que deseamos leer */
	...
	/* 6. Generamos la condición NoAck, para que el Master no responda y el slave solo envíe un byte */
